...
// Hash: 56c819387f9961d5c309ad34c500c93d49ad4ab1f1b1f7e74239f9fb2b58ad95
package enginev1
...
func (e *ExecutionPayload) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Offset (14) 'TimelockPrivatekey'
	dst = ssz.WriteOffset(dst, offset)
	if e.TimelockPrivatekey == nil {
		e.TimelockPrivatekey = new(RSAPrivateKey)
	}
	offset += e.TimelockPrivatekey.SizeSSZ()
...
func (e *ExecutionPayload) UnmarshalSSZ(buf []byte) error {
	...
	// Field (14) 'TimelockPrivatekey'
	{
		...
		if err = e.TimelockPrivatekey.UnmarshalSSZ(buf); err != nil {
			return err
		}
	}
	return err
...
func (e *ExecutionPayload) SizeSSZ() (size int) {
	...
	size += e.TimelockPrivatekey.SizeSSZ()
...
func (e *ExecutionPayloadHeader) UnmarshalSSZ(buf []byte) error {
	...
	// Offset (14) 'TimelockPrivatekey'
	if o14 = ssz.ReadOffset(buf[536:540]); o14 > size || o10 > o14 {
		return ssz.ErrOffset
	}
...
// MarshalSSZ ssz marshals the RSAPublicKey object
func (r *RSAPublicKey) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(r)
}

// MarshalSSZTo ssz marshals the RSAPublicKey object to a target array
func (r *RSAPublicKey) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf

	// Field (0) 'N'
	if size := len(r.N); size != 256 {
		err = ssz.ErrBytesLengthFn("--.N", size, 256)
		return
	}
	dst = append(dst, r.N...)

	// Field (1) 'E'
	dst = ssz.MarshalUint64(dst, r.E)

	return
}

// UnmarshalSSZ ssz unmarshals the RSAPublicKey object
func (r *RSAPublicKey) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size != 264 {
		return ssz.ErrSize
	...
	// Field (0) 'N'
	if cap(r.N) == 0 {
		r.N = make([]byte, 0, len(buf[0:256]))
	}
	r.N = append(r.N, buf[0:256]...)

	// Field (1) 'E'
	r.E = ssz.UnmarshallUint64(buf[256:264])

	return err
}

// SizeSSZ returns the ssz encoded size in bytes for the RSAPublicKey object
func (r *RSAPublicKey) SizeSSZ() (size int) {
	size = 264
	return
}

// HashTreeRoot ssz hashes the RSAPublicKey object
func (r *RSAPublicKey) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(r)
}

// HashTreeRootWith ssz hashes the RSAPublicKey object with a hasher
func (r *RSAPublicKey) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()
...
// MarshalSSZ ssz marshals the RSAPrivateKey object
func (r *RSAPrivateKey) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(r)
}

// MarshalSSZTo ssz marshals the RSAPrivateKey object to a target array
func (r *RSAPrivateKey) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(524)

	// Field (0) 'PublicKey'
	if r.PublicKey == nil {
		r.PublicKey = new(RSAPublicKey)
	}
	if dst, err = r.PublicKey.MarshalSSZTo(dst); err != nil {
		return
	}

	// Field (1) 'D'
	if size := len(r.D); size != 256 {
		err = ssz.ErrBytesLengthFn("--.D", size, 256)
		return
	}
	dst = append(dst, r.D...)
	...
	// Offset (2) 'Primes'
	dst = ssz.WriteOffset(dst, offset)
	for ii := 0; ii < len(r.Primes); ii++ {
		offset += 4
		offset += len(r.Primes[ii])
	}

	// Field (2) 'Primes'
	if size := len(r.Primes); size > 2 {
		err = ssz.ErrListTooBigFn("--.Primes", size, 2)
		return
	}
	{
		offset = 4 * len(r.Primes)
		for ii := 0; ii < len(r.Primes); ii++ {
			dst = ssz.WriteOffset(dst, offset)
			offset += len(r.Primes[ii])
		}
	...
	for ii := 0; ii < len(r.Primes); ii++ {
		if size := len(r.Primes[ii]); size > 256 {
			err = ssz.ErrBytesLengthFn("--.Primes[ii]", size, 256)
			return
		}
		dst = append(dst, r.Primes[ii]...)
	}

	return
}

// UnmarshalSSZ ssz unmarshals the RSAPrivateKey object
func (r *RSAPrivateKey) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 524 {
		return ssz.ErrSize
	...
	if err = r.PublicKey.UnmarshalSSZ(buf[0:264]); err != nil {
		return err
	}

	// Field (1) 'D'
	if cap(r.D) == 0 {
		r.D = make([]byte, 0, len(buf[264:520]))
	}
	r.D = append(r.D, buf[264:520]...)

	// Offset (2) 'Primes'
	if o2 = ssz.ReadOffset(buf[520:524]); o2 > size {
		return ssz.ErrOffset
	}
	...
	// Field (2) 'Primes'
	{
		...
		r.Primes = make([][]byte, num)
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if len(buf) > 256 {
				return ssz.ErrBytesLength
			}
			if cap(r.Primes[indx]) == 0 {
				r.Primes[indx] = make([]byte, 0, len(buf))
			}
			r.Primes[indx] = append(r.Primes[indx], buf...)
			return nil
		})
		if err != nil {
			return err
...
// SizeSSZ returns the ssz encoded size in bytes for the RSAPrivateKey object
func (r *RSAPrivateKey) SizeSSZ() (size int) {
	size = 524

	// Field (2) 'Primes'
	for ii := 0; ii < len(r.Primes); ii++ {
		size += 4
		size += len(r.Primes[ii])
	}

	return
}

// HashTreeRoot ssz hashes the RSAPrivateKey object
func (r *RSAPrivateKey) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(r)
}

// HashTreeRootWith ssz hashes the RSAPrivateKey object with a hasher
func (r *RSAPrivateKey) HashTreeRootWith(hh *ssz.Hasher) (err error) {
	indx := hh.Index()

	// Field (0) 'PublicKey'
	if err = r.PublicKey.HashTreeRootWith(hh); err != nil {
		return
	}
	...
	{
		subIndx := hh.Index()
		num := uint64(len(r.Primes))
		if num > 2 {
			err = ssz.ErrIncorrectListSize
			return
		}
		for _, elem := range r.Primes {
			{
				elemIndx := hh.Index()
				byteLen := uint64(len(elem))
				if byteLen > 256 {
